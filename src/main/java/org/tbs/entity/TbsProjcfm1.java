 ...
 */
@Entity
@Table(name = "tbs_projcfm1")
public class TbsProjcfm1 implements Serializable {
	...
	/**
	 * 表单编号=‘锡联担保审字’+会议日期+序号(seq.nextvalue)初始值国联提供序号是与cfm2表共用的，所以只能用seq函数生成:表单编号=‘锡联担保审字’+会议日期+序号(seq.nextvalue)初始值国联提供序号是与cfm2表共用的，所以只能用seq函数生成
	 */
	private String sn;
	...
	/**
	 * 会议日期，从projcfm0中带出,可修改，如果改了上面的SN也要自动修改:会议日期，从projcfm0中带出,可修改，如果改了上面的SN也要自动修改
	 */
	private Date meetingdate;

	/**
	 * 会议地点，从projcfm0中带出，可修改:会议地点，从projcfm0中带出，可修改
	 */
	private String meetingloc;

	/**
	 * 参会评委。从projcfm0_attend中带出一个list，作为字符串存在库中，可修改:参会评委。从projcfm0_attend中带出一个list，作为字符串存在库中，可修改
	 */
	...
	/**
	 * 上会类型【新项目、老项目新增、新项目变更、老项目变更、超时效项目、补议项目、其他】:上会类型【新项目、老项目新增、新项目变更、老项目变更、超时效项目、补议项目、其他】
	 */
	...
	/**
	 * tbs_projcfm1_opinion:会议决议录入表子表-审批意见
	 */
	private Set<TbsProjcfm1Opinion> tbsProjcfm1OpinionSet = new HashSet<TbsProjcfm1Opinion>(
			0);
	...
	public TbsProjcfm1(int id, String sn, boolean valid, Date timestampInput,
			Date timestampUpdate, int keyinId, String memo, boolean del,
			String projSn, String arolename, String brolename,
			Date meetingdate, String meetingloc, String attend, int tobe,
			int actbe, int nobe ,int agree, int opoose, int waiver, int debarb,
	...
	@OneToMany(fetch = FetchType.LAZY, mappedBy = "tbsProjcfm1")
	public Set<TbsProjcfm1Bizvt> getTbsProjcfm1BizvtSet() {
		return tbsProjcfm1BizvtSet;
	}
	
	public void setTbsProjcfm1OpinionSet(Set<TbsProjcfm1Opinion> tbsProjcfm1OpinionSet) {
		this.tbsProjcfm1OpinionSet = tbsProjcfm1OpinionSet;
	}
	...
	public String toString() {
		return "TbsProjcfm1 [id=" + id + ",sn=" + sn + ",valid=" + valid
				+ ",timestampInput=" + timestampInput + ",timestampUpdate="
				+ timestampUpdate + ",keyinId=" + keyinId + ",memo=" + memo
				+ ",del=" + del + ",projSn=" + projSn + ",arolename="
				+ arolename + ",brolename=" + brolename + ",meetingdate="
				+ meetingdate + ",meetingloc=" + meetingloc + ",attend="
				+ attend + ",tobe=" + tobe + ",actbe=" + actbe + ",nobe=" + nobe + ",agree="
				+ agree + ",opoose=" + opoose + ",waiver=" + waiver
				+ ",debarb=" + debarb + ",meetingtype=" + meetingtype
				+ ",vfaloc=" + vfaloc + ",vnfaloc=" + vnfaloc + ",votloc="
				+ votloc + ",vtotloc=" + vtotloc + ",periodCfm=" + periodCfm
				...
				+ ",tbsProjcfm1BizvtSet=" + tbsProjcfm1BizvtSet + ",tbsProjcfm1OpinionSet=" + tbsProjcfm1OpinionSet
