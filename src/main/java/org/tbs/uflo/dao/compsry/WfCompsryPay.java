...
@Component
public class WfCompsryPay extends HibernateDao {
	...
	public String StartWF(Map<String, Object> params) throws Exception {
		...
		if (null != params) {
			...
			String docid = Integer.toString((int) params.get("compsrypayid")); 
			...
			info.setTag("compsrypay"); // 对流程实例打个标记，以备后续使用
			...
			ProcessInstance pi = processClient.startProcessByName("compsrypay", info);
	...
	public void ApprWF(Map<String, Object> params) throws Exception {
		...
		try {
			...
			if (outcome.equals("通过")){			
				...
				i=0;
				variables.put("reject", i);	
				Msgpkt.setTitle("【代偿请款】已完成修改！");
				Msgpkt.setContent("您发送的【代偿请款】\n【项目名称："+projname+"】\n已经在【"+nodename+"】完成修改\n审批意见: " + comment+"\n日期："+today);
				SendMsg.send(Msgpkt);
			}else if(outcome.equals("确认")){
			session.update(tprls);
			Msgpkt.setTitle("【代偿请款】已通过！"+nodename+"环节");
			Msgpkt.setContent("您发送的【代偿请款】\n【项目名称："+projname+"】\n已经在【"+nodename+"】环节完成\n审批意见: " + comment+"\n日期："+today);
			SendMsg.send(Msgpkt);
		}
		} finally {
