...
@Component
public class WfCompsry extends HibernateDao {
	...
	public String StartWF(Map<String, Object> params) throws Exception {
		...
		if (null != params) {
			...
			String docid = Integer.toString((int) params.get("compsryid")); 
			...
			info.setTag("compsry"); // 对流程实例打个标记，以备后续使用
			...
			ProcessInstance pi = processClient.startProcessByName("compsry", info);
	...
	public void ApprWF(Map<String, Object> params) throws Exception {
		...
		try {
			...
			if (outcome.equals("通过")){			
				Msgpkt.setTitle("【代偿审议】审批已通过！");
				Msgpkt.setContent("您发送的【代偿审议】\n【项目名称："+projname+"】\n已经通过【"+nodename+"】审批！\n审批意见: " + comment+"\n日期："+today);
				SendMsg.send(Msgpkt);
			}else if (outcome.equals("驳回")) { 
				tprls.setValid(3);
				session.update(tprls);
				Msgpkt.setTitle("【代偿审议】审批已驳回！");
				Msgpkt.setContent("您发送的【代偿审议】\n【项目名称："+projname+"】\n已经被驳回,驳回节点为【"+nodename+"】\n审批意见: " + comment+"\n日期："+today);
				SendMsg.send(Msgpkt);				
			}else if(outcome.equals("修改确认")){
				tprls.setValid(2);
				session.update(tprls);
				Msgpkt.setTitle("【代偿审议】已完成修改！");
				Msgpkt.setContent("您发送的【代偿审议】\n【项目名称："+projname+"】\n已经被【"+nodename+"】\n审批意见: " + comment+"\n日期："+today);
				SendMsg.send(Msgpkt);
			}
		} finally {
