...
@Component
public class WfChangeRole extends HibernateDao {
	...
	public String StartWF(Map<String, Object> params) throws Exception {
		...
		if (null != params) {
			...
			String sql="call p_changerole_othdept(1,"+docid+")";
			SQLQuery query=session.createSQLQuery(sql);
			String othdept=query.uniqueResult().toString(); 
			...
			info.setTag("changerole"); // 对流程实例打个标记，以备后续使用
			Map<String, Object> variables = new HashMap<String, Object>();
			variables.put("othdept", othdept); 
			variables.put("projSn", tpcr.getProjSn());
			variables.put("projName", tpcr.getTbsProj().getProjName());
			variables.put("projId", projid);
			info.setVariables(variables); 
			ProcessInstance pi = processClient.startProcessByName("changerole", info);
			Long processInstanceId = pi.getId();
			result = "提交审批成功！";
			try {
				tpcr.setBy1(processInstanceId.toString());  //把processInstanceId放入单据的by1内
				if (!pi.getCurrentNode().equals("结束")) {  // 开始流程后直接执行action并结束时需要做这个判断
					tpcr.setValid(2);    //单据状态改为审批中 
				} else {
					tpcr.setValid(1); 
				}
				session.update(tpcr);				
