...
!function(self, arg) {
	...
	if (aprv == 2) {
		var rq = new Date();
		var dataSet = view.get("#dataSetTbsProjCompsry");
		var autoform = view.get("#AutoFormCompsry");
		...
		view.get("#btnSave").set("action","saveCompsry");
		...
		var rlsfaloc = "${request.getParameter('rlsfaloc')}";
		var rlsnfaloc = "${request.getParameter('rlsnfaloc')}";
		var rlsotloc = "${request.getParameter('rlsotloc')}";
		var rlstotloc = "${request.getParameter('rlstotloc')}";
		dataSet.getData("#").set("tbsProjundwrt.rlsfaloc",rlsfaloc);
		dataSet.getData("#").set("tbsProjundwrt.rlsnfaloc",rlsnfaloc);
		dataSet.getData("#").set("tbsProjundwrt.rlsotloc",rlsotloc);
		dataSet.getData("#").set("tbsProjundwrt.rlstotloc",rlstotloc);
		...
		var compsryvalid = "${request.getParameter('compsryvalid')}";
		if (compsryvalid != "0"){
			view.get("#dataSetTbsProjCompsry").set("readOnly",true);
			view.get("#bizmemo").set("visible",true);
			view.get("#riskmemo").set("visible",true);
			view.get("#btnSubmit").set("visible", false);
		...
		view.get("#dataSetTbsProjCompsry").set("parameter", compsryid).flush();
	} else {
		var businessId = "${request.getParameter('businessId')}";  
		view.get("#dataSetTbsProjCompsry").set("parameter", businessId).flush();  // 这里为了立刻让dataset有值只能用flush，不能用flushasync
		var dataSet = view.get("#dataSetTbsProjCompsry").getData("#"); 
		var projid = dataSet.get("tbsProj.id"); 
		var compsryvalid = dataSet.get("valid");
		...
		view.get("#ajaxactionGetUfloNode").set("parameter",params).execute(
				function(nodename){
				if (nodename == "决定是否召开风险管理会议"){
					view.get("#dataSetTbsProjCompsry").set("readOnly",false);
					view.get("#outcome").set("label","是否召开风险管理会议");
				...
				if (nodename =="业务审核委员会秘书录入会议决议"){
					...
					view.get("#date").set("readOnly",true);
					view.get("#bizmemo").set("visible",true);
					view.get("#bizmemo").set("readOnly",false);
...
!function(self, arg, saveCompsry) {
	if(aprv==2||saveCompsry.get("hasUpdateData")){
		saveCompsry.execute();
	}
...
!function(self, arg, dataSetTbsProj, dataSetTbsProjCompsry, ajaxactionStartProcess, saveCompsry) {
	var projid = dataSetTbsProj.getData("#").get("id");
	var compsryid = dataSetTbsProjCompsry.getData("#").get("id");
	var params ={
			projid : projid,
			compsryid : compsryid
	};
	if(saveCompsry.get("hasUpdateData")){
		saveCompsry.execute();
	}
	...
	dataSetTbsProjCompsry.flushAsync();
	dataSetTbsProjCompsry.set("readOnly",true);
...
!function(self,arg,autoformOpinion,saveCompsry,ajaxactionApprSubmit) {
	...
	if (outcome == "驳回" && !comment) {
		...
		if (outcome == "修改确认" || outcome == "确认" || outcome == "是" ||outcome == "否") {
			saveCompsry.execute();
		};
