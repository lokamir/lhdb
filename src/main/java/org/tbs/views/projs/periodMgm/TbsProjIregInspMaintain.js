...
!function(self, arg, autoformCondition, dataSetTbsProj, btnAdd) {
...
!function(self,arg,iFrameProjDetails,iFrameProjAttach,dialogProjDetails,dataSetTbsProj){
	...
	var detailpath="org.tbs.views.projs.periodMgm.DetailDialog12.d?menu=IregInsp&aprv=0&projid=" + projid 
	+ "&projid=" + projid 
	+ "&keyinid=" + keyinid 
	+ "&projsn=" + projsn 
	+ "&projName=" + projName 
...
!function(self,arg,iFrameProjDetails,iFrameProjAttach,dialogProjDetails,dataSetTbsProj){
	...
	var projid = dataSet.get("id");
	var projsn = dataSet.get("sn");
	var detailpath="org.tbs.views.projs.periodMgm.DetailDialog12.d?menu=IregInsp&aprv=2&projid=" + projid 
			+ "&projid=" + projid
			+ "&projsn=" + projsn
			+ "&dt=" + new Date();
	iFrameProjDetails.set("path",detailpath);
...
!function(self,arg,dataSetTbsProj,ajaxDel){
	...
	var projcheckvalid = dataSet.get("tbsProjCheckSet").current.get("valid");
	if (projcheckvalid == 0) {
		dorado.MessageBox.confirm("您真的要删除此【不定期检查记录】吗？\n此操作无法恢复！",
			{title:"趣博信息科技",
