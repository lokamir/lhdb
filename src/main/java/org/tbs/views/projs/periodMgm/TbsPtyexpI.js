...
function GetMenu(button){
	...
	if (view.get("#dataSetTbsProj").getData("#.tbsProjundwrtSet.id")) {
		var undwrtid = view.get("#dataSetTbsProj").getData(
				"#.#tbsProjundwrtSet.id");
		var projdetailspath = "org.tbs.views.projs.periodMgm.DetailDialog34.d?aprv="+aprv+"&io=1&projid="+ projid
				+ "&undwrtid="+ undwrtid
				+ "&ptyexpid="+ ptyexpid
				+"&valid="+valid
				+ "&menu=PtyexpI" 
...
!function(updateActionSaveTbsPtyexp){
	...
	if(valid == 0){
		dorado.MessageBox.confirm("您真的要删除此【费用收取单据】吗？\n此操作无法恢复！", {
			title : "趣博信息科技",
		...
		dorado.MessageBox.alert("无法删除！\n此【费用收取单】已经通过审核，或正在审核中！",{title:"趣博信息科技"});
	}
};
